/* =========================================================
   ...
   実際の伸び（ウェーブ 0 → 上限到達まで）:
     敵の湧く量   1.5 → 90 匹/秒   （約 60 倍）
     敵弾の上限   16  → 34 発      （約 2.1 倍）
     敵弾の速さ   112 → 158 px/秒  （約 1.4 倍）
     撃ってくる敵  6  → 16 体      （約 2.7 倍）
     自機の火力   約 10 → 約 190 DPS（約 19 倍）  ← 一番伸びる

   敵の数だけが桁で増え、危険はゆっくり増え、火力はそれを上回る。
   この 3 段構えが「難しくなっているのに、どんどん楽になる」感覚を作る。

   難易度側の 4 つの数値（CFG.curve）はどれも上限つきなので、
   何分やっても際限なく難しくなることはない。

   一方、以下は最後まで固定する。ここを動かすと質的に別のゲームになるため。

     ・敵の HP … 増えるのは「数」であって「硬さ」ではない。
                 硬くすると武器を強くした手応えがそのまま消える。
