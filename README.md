...
|---|---|---|---|
| 100 秒 | 22 | 97% | 315px / 336px |
| 200 秒 | 36 | 77% | 197px |
| 350 秒 | 93 | 72% | 160px |

序盤はほぼ無風、終盤は画面の 3 割が危険地帯になる。
...
そこで **実効ウェーブ = 経過ウェーブ ＋ 装備の充実度 × 9** とし、
フル装備なら 9 ウェーブぶん先の脅威と戦うことにしてある。
...
**もっと歯ごたえが欲しい**なら、まず `threatFromPower`（既定 9）を上げる。
...
**難しすぎる**なら `threatFromPower` を 4〜5 に下げるか、
